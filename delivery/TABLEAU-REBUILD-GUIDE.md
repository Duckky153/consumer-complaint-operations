...
Make Overview and Investigation, each 1200 × 850. Use a readable heading and a left filter rail on both pages. Group the five metrics into three groups: complaints, not timely (count and rate), and relief (count and mix). Overview contains the two monthly charts. Investigation stacks full-width issue and sub-issue tables with plain count/rate headers. Format row headers with sufficient width and wrapping; use Fit Width for the lists. State source limitations and the proposed internal validation step without requiring hover. See evidence/screenshots/tableau-readable/ for the current layout. Use 14-point chart labels and 17-point section headings. Format Parameters uses the parameter-ctrl and parameter-ctrl-title styles; compact dropdown values remain native-size, so the delivered workbook adds four unfiltered Current selection text sheets for larger readbacks. Long selections receive a line break at a word boundary. Use List mode for January sensitivity and alias the third underlying value as “Exclude top 2 groups”; its stored value and calculation remain unchanged.
